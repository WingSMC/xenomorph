...
#[derive(Debug, Clone)]
pub struct XenoRegistry(Arc<RwLock<ModuleRegistry>>, PathBuf);
...
impl XenoRegistry {
    fn get_root() -> Result<PathBuf, ModuleError> {
        let config = Config::get();
        let file_path = config.workdir.join(&config.parser.path);
        file_path.canonicalize().map_err(|e| ModuleError {
            message: format!("Cannot resolve entry file '{}': {}", file_path.display(), e,),
            module_path: file_path.display().to_string(),
        })
    }

    pub fn init(root_opt: Option<&Path>) -> Result<XenoRegistry, ModuleError> {
        let root = if root_opt.is_some() {
            root_opt.unwrap().to_path_buf()
        } else {
            let r = XenoRegistry::get_root();
            match r {
                Ok(p) => p,
                Err(e) => return Err(e),
            }
    ...
    pub fn load_module(&self, file_abs: &Path) -> Vec<ModuleError> {
        ...
        {
            let mut reg = self.0.write().unwrap();
            reg.modules.insert(
                module_path.clone(),
                ModuleData {
        ...
        for (segments, _joined) in &import_paths {
            ...
            match self.resolve_import(&segments_str, file_abs) {
                Ok((_, import_abs)) => {
                    ...
                    let sub_errors = self.load_module(&import_abs);
    ...
    pub fn build_declaration_cache(&self) -> HashMap<String, DeclarationInfo> {
        let reg = self.0.read().unwrap();
    ...
    pub fn resolve_import(
        ...
        let current_dir = current_file_abs.parent().unwrap_or(self.1.as_path());
        ...
        let abs_path = current_dir
            ...
            .or_else(|_| {
                ...
                Ok::<PathBuf, std::io::Error>(
                    current_dir.join(format!("{}.xen", relative_from_current)),
                )
            })
            .unwrap();
    ...
    pub fn load_workspace(entry_file: &Path) -> (Option<XenoRegistry>, Vec<ModuleError>) {
        let workspace_root = entry_file.parent();
        let registry_res = XenoRegistry::init(workspace_root);
        let reg = match registry_res {
            Ok(r) => r,
            Err(e) => {
                return (None, vec![e]);
            }
        ...
        let errors = reg.load_module(entry_file);
        (Some(reg), errors)
    }
